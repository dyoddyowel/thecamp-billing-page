const soap = require('./soapClient');
const service = "ClientService";
const args = {};

const buildArguments = (siteID) => {
    let params = {
        "Request": {
            "Content-Type": "application/json",
            "API-key": 'process.env.MINDBODY_API_KEY',
            "SourceCredentials": {
                "SourceName": 'OnePercentNutrition',
                "Password": 'gj9RdLNeymPV7TK4kusMrzG7NYw=',
                "SiteIDs": {
                    "int": siteID
                }
            },
            "UserCredentials": {
                "SiteIDs": {
                    "int": siteID
                },
                "Username": "Alejandra@thecamptc.com",
                "Password": "fitness102",
            }
        }
    }
    return params;
}

const getRequiredFields = async (params) => {
    let client = await soap(service);
    return new Promise ((resolve, reject) => {
        client.GetRequiredClientFields(params, (err, result) => {
            if(err) {
                console.log(err);
            }
            let requiredFields = result['GetRequiredClientFieldsResult']['RequiredClientFields'];
            return resolve(requiredFields);
        })     
    })
}

const getExistingClient = async (params, email) => {
    let client = await soap(service);
    params['Request']['SearchText'] = email;
    return new Promise ((resolve, reject) => {
        client.GetClients(params, async (err, result) => {
            if(err) {
                console.log(err);
            }
             return resolve(result['GetClientsResult']['Clients']);
        })
    })
}	

const convertToObject = (fields) => {
    console.log(fields);
    let rFields = {};
    for(let i of fields) {
        rFields[i] = "";
    }
    return rFields;
}

const addClient = async (args, fields) => {
    let params = {
        Request: {
            'Content-Type': 'application/json',
            'API-key': process.env.MINDBODY_API_KEY,
            SourceCredentials: args.Request.SourceCredentials,
            Clients: {
                Client: fields
            }
        }
    }
    console.log(params)
    let client = await soap(service);
    return new Promise((resolve, reject) => {
        client.AddOrUpdateClients(params, (err, result) => {
                if(err) {
                    console.log(err);
                }
                console.log("clients result", result['AddOrUpdateClientsResult']);
                if(result['AddOrUpdateClientsResult']['Clients']['Client']) {
                    return resolve(result['AddOrUpdateClientsResult']['Clients']['Client']);
                } else {
                    return reject(err);
                }
                
        })
    });
}

const looper = async () => {
    let client = await soap(service);
    let keys = Object.keys(locations);
    let big_arr = [];
    return new Promise((resolve, reject) => {

        keys.forEach((key) => {
            let obj = {};
            let arr = [];
            const args = buildArguments(locations[key]['siteID']);
            obj['siteID'] = locations[key]['siteID'];
            names.forEach(async (name) => {
                args['Request']['SearchText'] = name;
                try {
                    await client.GetClients(args, (err, result) => {
                        if(err) {
                            throw(err);
                        }
                         let soap_result = result['GetClientsResult']['Clients'];
                         arr.push(soap_result);
                    })
                } catch(error) {
                    console.log("error on site id", obj['siteID']);
                }
            });
            obj['searches'] = arr;
            big_arr.push(obj);
        });
        return resolve(big_arr);
    })
}

const rend = async (arr) => {
    let loop = await looper();
    console.log(loop[0])
    return loop;
}

module.exports.getRequiredFields = getRequiredFields;
module.exports.addClient = addClient;  
module.exports.getExistingClient = getExistingClient;  
module.exports.buildArguments = buildArguments;
module.exports.looper = looper;
module.exports.rend = rend;



let names = [
    "Prisma paredes",
    "Laura garcia",
    "Alejandra zayas",
    "Estefanía Gómez Ruiz",
];


const locations = {
    'Alexandria': 
      {
          siteID: '830783',
          programID: '101036',
          tagID: '3226',
          pixelID: ''
  },
  'Anaheim': 
      {
          siteID: '172426',
          programID: '11603',
          tagID: '3228',
      pixelID: '1019141084810848'
  },
  'Arcadia': 
      {
          siteID: '505838',
          programID: '101134',
          tagID: '3230',
      pixelID: '170049590227137'
  },
  'Arlington': 
      {
          siteID: '469848',
          programID: '10336',
          tagID: '3232',
      pixelID: '305596673130406'
  },
  'Bakersfield': 
      {
          siteID: '539757',
          programID: '103110',
          tagID: '3234',
      pixelID: '124038588258655'
  },
  'Bell': 
      {
          siteID: '273870',
          programID: '10564',
          tagID: '3236',
          pixelID: '1392379664110280'
  },
  'Burbank': 
      {
          siteID: '264885',
          programID: '10846',
          tagID: '3238',
             pixelID: '179752492676186'
  },
  
  'Carson': 
      {
          siteID: '298511',
          programID: '10309',
          tagID: '3242',
          pixelID: ''
  },
  'Calexico': 
      {
          siteID: '620570',
          programID: '100088',
          tagID: '3240',
          pixelID: ''
  },
  'Chatsworth': 
      {
          siteID: '3324',
          programID: '10486',
          tagID: '3244',
          pixelID: ''
  },
  'Chino': 
      {
          siteID: '143922',
          programID: '11050',
          tagID: '3246',
          pixelID: ''
  },
  'Chula-Vista': 
      {
          siteID: '248377',
          programID: '10298',
          tagID: '3248',
          pixelID: ''
  },
  'Clearwater': 
      {
          siteID: '698580',
          programID: '10291',
          tagID: '3250',
          pixelID: ''
  },
  'Colorado-Springs': 
      {
          siteID: '735761',
          programID: '101074',
          tagID: '3254',
          pixelID: ''
  },
  'Corona': 
      {
          siteID: '233',
          programID: '10652',
          tagID: '3256',
          pixelID: ''
  },
  'Costa-Mesa': 
      {
          siteID: '221376',
          programID: '10319',
          tagID: '3258',
          pixelID: ''
  },
  'Cypress': 
      {
          siteID: '278263',
          programID: '10420',
          tagID: '3260',
          pixelID: ''
  },
  'Davie': 
      {
          siteID: '257960',
          programID: '10281',
          tagID: '3262',
          pixelID: ''
  },
  'Downey': 
      {
          siteID: '273870',
          programID: '10564',
          tagID: '3264',
          pixelID: ''
  },
  'Draper': 
      {
          siteID: '414648',
          programID: '10331',
          tagID: '3266',
          pixelID: ''
  },
  'East-Los-Angeles': 
      {
          siteID: '253486',
          programID: '10585',
          tagID: '3268',
          pixelID: ''
  },
  'El-Cajon': 
      {
          siteID: '347909',
          programID: '10277',
          tagID: '3270',
          pixelID: ''
  },
  'El-Monte': 
      {
          siteID: '264885',
          programID: '10847',
          tagID: '3272',
          pixelID: ''
  },
  'Escondido': 
      {
          siteID: '581912',
          programID: '101068',
          tagID: '3274',
          pixelID: ''
  },
  'Fontana': 
      {
          siteID: '143931',
          programID: '10979',
          tagID: '3276',
          pixelID: ''
  },
  'Fort-Worth': 
      {
          siteID: '834435',
          programID: '10474',
          tagID: '3278',
          pixelID: ''
  },
  'Galerias': 
      {
          siteID: '557811',
          programID: '100109',
          tagID: '3282',
          pixelID: ''
  },
  'Gilbert': 
      {
          siteID: '589611',
          programID: '10527',
          tagID: '3284',
          pixelID: ''
  },
  'Hemet': 
      {
          siteID: '-291840',
          programID: '10628',
          tagID: '3286',
          pixelID: ''
  },
  'High-Desert': 
      {
          siteID: '191573',
          programID: '10438',
          tagID: '3288',
          pixelID: ''
  },
  'Highland-Heights': 
      {
          siteID: '912776',
          programID: '10335',
          tagID: '3290',
          pixelID: ''
  },
  'Houston': 
      {
          siteID: '-879013',
          programID: '10332',
          tagID: '3292',
          pixelID: ''
  },
  'Humble': 
      {
          siteID: '682932',
          programID: '102094',
          tagID: '3294',
          pixelID: ''
  },
  'Huntington-Beach': 
      {
          siteID: '441683',
          programID: '10331',
          tagID: '3296',
          pixelID: ''
  },
  'Inglewood': 
      {
          siteID: '740074',
          programID: '102129',
          tagID: '3298',
          pixelID: ''
  },
  'Irvine': 
      {
          siteID: '228344',
          programID: '10324',
          tagID: '3300',
          pixelID: ''
  },
  'Jacksonville': 
      {
          siteID: '715841',
          programID: '10352',
          tagID: '3302',
          pixelID: ''
  },
  'Kearny-Mesa': 
      {
          siteID: '528419',
          programID: '10290',
          tagID: '3304',
          pixelID: ''
  },
  'La-Mirada': 
      {
          siteID: '273870',
          programID: '10564',
          tagID: '3306',
          pixelID: ''
  },
  'La-Puente': 
      {
          siteID: '136390',
          programID: '10248',
          tagID: '3308',
          pixelID: ''
  },
  'Laguna-Hills': 
      {
          siteID: '221032',
          programID: '10309',
          tagID: '3310',
          pixelID: ''
  },
  'Lake-Elsinore': 
      {
          siteID: '963',
          programID: '10328',
          tagID: '3312',
          pixelID: ''
  },
  'Lake-Forest': 
      {
          siteID: '274470',
          programID: '10335',
          tagID: '3314',
          pixelID: ''
  },
  'Lancaster': 
      {
          siteID: '149049',
          programID: '10642',
          tagID: '3316',
          pixelID: ''
  },
  'Long-Beach': 
      {
          siteID: '245950',
          programID: '10368',
          tagID: '3318',
          pixelID: ''
  },
  'Los-Angeles': 
      {
          siteID: '300527',
          programID: '10223',
          tagID: '3320',
          pixelID: ''
  },
  'Las-Vegas': 
      {
          siteID: '678753',
          programID: '101199',
          tagID: '4562',
          pixelID: ''
  },
  'Las-Vegas-East': 
      {
          siteID: '974723',
          programID: '101194',
          tagID: '4560',
          pixelID: ''
  },
  'Manteca': 
      {
          siteID: '826587',
          programID: '101191',
          tagID: '4570',
          pixelID: ''
  },
  'Mid-Cities': 
      {
          siteID: '764893',
          programID: '10257',
          tagID: '3322',
          pixelID: ''
  },
  'McKinney': 
      {
          siteID: '568503',
          programID: '100087',
          tagID: '3324',
          pixelID: ''
  },
  'Mckinney': 
      {
          siteID: '568503',
          programID: '100087',
          tagID: '3324',
          pixelID: ''
  },
  'Menifee': 
      {
          siteID: '228342',
          programID: '10317',
          tagID: '3326',
          pixelID: ''
  },
  'Mesa': 
      {
          siteID: '497618',
          programID: '11430',
          tagID: '3328',
          pixelID: ''
  },
  'Mid-Phoenix': 
      {
          siteID: '822381',
          programID: '101057',
          tagID: '3330',
          pixelID: ''
  },
  'Milwaukie': 
      {
          siteID: '707289',
          programID: '10358',
          tagID: '3332',
          pixelID: ''
  },
  'Minnesota': 
      {
          siteID: '441415',
          programID: '101033',
          tagID: '3334',
          pixelID: ''
  },
  'Miramar': 
      {
          siteID: '347920',
          programID: '10265',
          tagID: '3336',
          pixelID: ''
  },
  'Miramar-CA': 
      {
          siteID: '347920',
          programID: '10265',
          tagID: '3336',
          pixelID: ''
  },
  'Modesto': 
      {
          siteID: '289688',
          programID: '11287',
          tagID: '3338',
          pixelID: ''
  },
  'Monrovia': 
      {
          siteID: '458660',
          programID: '10825',
          tagID: '3340',
          pixelID: ''
  },
  'Monterey': 
      {
          siteID: '522220',
          programID: '103107',
          tagID: '3342',
          pixelID: ''
  },
  'Moreno-Valley': 
      {
          siteID: '276772',
          programID: '10680',
          tagID: '3344',
          pixelID: ''
  },
  'North-Dallas': 
      {
          siteID: '837070',
          programID: '10322',
          tagID: '3346',
          pixelID: ''
  },
  'Northridge': 
      {
          siteID: '264885',
          programID: '10848',
          tagID: '3348',
          pixelID: ''
  },
  'Oceanside': 
      {
          siteID: '192329',
          programID: '10307',
          tagID: '3350',
          pixelID: ''
  },
  'Orlando': 
      {
          siteID: '783641',
          programID: '101077',
          tagID: '3352',
          pixelID: ''
  },
  'Oxnard': 
      {
          siteID: '241280',
          programID: '10645',
          tagID: '3354',
          pixelID: ''
  },
  'Palm-Desert': 
      {
          siteID: '726432',
          programID: '10646',
          tagID: '3356',
          pixelID: ''
  },
  'Parma': 
      {
          siteID: '906879',
          programID: '100087',
          tagID: '3358',
          pixelID: ''
  },
  'Pasadena': 
      {
          siteID: '228343',
          programID: '10306',
          tagID: '3360',
          pixelID: ''
  },
  'Peoria': 
      {
          siteID: '748408',
          programID: '103083',
          tagID: '3362',
          pixelID: ''
  },
  'Perris': 
      {
          siteID: '963',
          programID: '10328',
          tagID: '3364',
          pixelID: ''
  },
  'Phoenix': 
      {
          siteID: '-314525',
          programID: '10333',
          tagID: '3366',
          pixelID: ''
  },
  'Pico-Rivera': 
      {
          siteID: '253486',
          programID: '10585',
          tagID: '3368',
          pixelID: ''
  },
  'Rancho': 
      {
          siteID: '143922',
          programID: '11058',
          tagID: '3370',
          pixelID: ''
  },
  'Redlands': 
      {
          siteID: '143945',
          programID: '10713',
          tagID: '3372',
          pixelID: ''
  },
  'Richardson': 
      {
          siteID: '353209',
          programID: '10306',
          tagID: '3374',
          pixelID: ''
  },
  'Riverside': 
      {
          siteID: '143948',
          programID: '10685',
          tagID: '3376',
          pixelID: ''
  },
  'Round-Rock': 
      {
          siteID: '637731',
          programID: '100083',
          tagID: '3378',
          pixelID: ''
  },
  'San-Antonio': 
      {
          siteID: '442521',
          programID: '10314',
          tagID: '3380',
          pixelID: ''
  },
  'San-Bernardino': 
      {
          siteID: '663134',
          programID: '102139',
          tagID: '3382',
          pixelID: ''
  },
  'San-Dimas': 
      {
          siteID: '274359',
          programID: '10343',
          tagID: '3384',
          pixelID: ''
  },
  'San-Fernando': 
      {
          siteID: '264885',
          programID: '10849',
          tagID: '3386',
          pixelID: ''
  },
  'San-Jose': 
      {
          siteID: '461323',
          programID: '102124',
          tagID: '3388',
          pixelID: ''
  },
  'San-Juan-Capistrano': 
      {
          siteID: '819476',
          programID: '100073',
          tagID: '3390',
          pixelID: ''
  },
  'Santa-Ana': 
      {
          siteID: '542643',
          programID: '10310',
          tagID: '3392',
          pixelID: ''
  },
  'Santa-Barbara': 
      {
          siteID: '420012',
          programID: '100072',
          tagID: '3394',
          pixelID: ''
  },
  'Santa-Clarita': 
      {
          siteID: '264885',
          programID: '10850',
          tagID: '3396',
          pixelID: ''
  },
  'Santa-Maria': 
      {
          siteID: '570071',
          programID: '100066',
          tagID: '3398',
          pixelID: ''
  },
  'Sarasota': 
      {
          siteID: '525810',
          programID: '10251',
          tagID: '3400',
          pixelID: ''
  },
  'Silverlake': 
      {
          siteID: '833754',
          programID: '101033',
          tagID: '3402',
          pixelID: ''
  },
  'Simi-Valley': 
      {
          siteID: '712144',
          programID: '102175',
          tagID: '3404',
          pixelID: ''
  },
  'South-Bay': 
      {
          siteID: '',
          programID: '',
          tagID: '3406',
          pixelID: ''
  },
  'South-Fort-Worth': 
      {
          siteID: '761241',
          programID: '100078',
          tagID: '3408',
          pixelID: ''
  },
  'Stockton': 
      {
          siteID: '994773',
          programID: '101265',
          tagID: '3410',
          pixelID: ''
  },
  'Sunrise': 
      {
          siteID: '694338',
          programID: '11299',
          tagID: '3412',
          pixelID: ''
  },
  'Surprise': 
      {
          siteID: '863734',
          programID: '10275',
          tagID: '3414',
          pixelID: ''
  },
  'Suwanee': 
      {
          siteID: '402019',
          programID: '10392',
          tagID: '3416',
          pixelID: ''
  },
  'Temecula': 
      {
          siteID: '142187',
          programID: '10992',
          tagID: '3418',
          pixelID: ''
  },
  'Thousand-Oaks': 
      {
          siteID: '198685',
          programID: '10663',
          tagID: '3420',
          pixelID: ''
  },
  'Vancouver': 
      {
          siteID: '477491',
          programID: '',
          tagID: '3422',
          pixelID: ''
  },
  'Van-Nuys': 
      {
          siteID: '264885',
          programID: '10851',
          tagID: '3424',
          pixelID: ''
  },
  'Ventura': 
      {
          siteID: '633911',
          programID: '104211',
          tagID: '3426',
          pixelID: ''
  },
  'West-Chula-Vista': 
      {
          siteID: '616363',
          programID: '10303',
          tagID: '3428',
          pixelID: ''
  },
  'West-Covina': 
      {
          siteID: '264885',
          programID: '10852',
          tagID: '3430',
          pixelID: ''
  },
  'Westminster': 
      {
          siteID: '275479',
          programID: '10398',
          tagID: '3432',
          pixelID: ''
  },
  'Wheeling': 
      {
          siteID: '909945',
          programID: '100082',
          tagID: '3434',
          pixelID: ''
  },
  'Woodland-Hills': 
      {
          siteID: '345827',
          programID: '10634',
          tagID: '3436',
          pixelID: ''
  },
  'Zona-Dorada ': 
      {
          siteID: '624525',
          programID: '10350',
          tagID: '3438',
          pixelID: ''
  },
  'Zona-Rio-Tijuana': 
      {
          siteID: '551868',
          programID: '100102',
          tagID: '3440',
          pixelID: ''
  },
  
  }
